...
function WaterfallVertical({ app, style, reference, options }) {
  ...
  function getData() {
    Utils.createHyperCubeByQlikId(app, reference.id, (newData, vis) => {
      ...
      const max = Math.max(
        ...qDataPages[0].qMatrix[0].map((item) => item.qNum),
      );
...
WaterfallVertical.defaultProps = {
  app: null,
  style: {},
  options: appearance.waterfall(),
};

WaterfallVertical.propTypes = {
...
export default WaterfallVertical;
