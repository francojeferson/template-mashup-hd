...
function CustomTable({
    ...
    useEffect(() => {
        ...
        if (!reference.radioButtonControl || !reference.columnToHideRadioButton) return;
    ...
    useEffect(() => {
        if (!reference.maxColumnsToShow || data.headerData.length === 0 || hideColumn) return;
        const cancelArray = [];
    ...
    useEffect(() => {
        ...
        if (data.visualizationModel && field && searchWord) {
            ...
            const arg = `=if(wildmatch([${field}], '${searchWord + '*'}') > 0, [${field}])`;
            const softPatches = {
            ...
            data.visualizationModel.model.enigmaModel.applyPatches([softPatches], true);
    ...
    useEffect(() => {
        ...
        Utils.createHyperCubeByQlikIdToTable(app, reference.id, th, (tableData, vis, title, id) => {
            app.visualization.get(id).then((visualizationModel) => {
                ...
                const field = vis.model.layout.qHyperCube.qDimensionInfo[0].qGroupFieldDefs[0];
    ...
    useEffect(() => {
        ...
        setEventAdded(true);
        tableRef.current.addEventListener('scroll', throttle((evt) => {
            const table = evt.target;
            const difTableHeight = table.scrollHeight - table.scrollTop - table.offsetHeight;
            if (difTableHeight < 100) {
                data.visualizationModel.model.getProperties().then((res) => {
                    res.qHyperCubeDef.qInitialDataFetch[0].qHeight += reference.loadMore || 20;
    ...
    return (
        <div ref={tableRef} style={{ maxHeight: '700px' }} className={searchOpened ? 'tableWrapper active-search' : 'tableWrapper'}>
            {searchOpened && (
                <div className="custom-table-search-header">
                    ...
                    <div>
                        ...
                        <div>
                            <input defaultValue={searchWord} type="text" placeholder="Pesquise por descrição" onChange={searchFunction} />
                        </div>
                    </div>
                </div>
            )}
                ...
                <thead>
                    <tr>
                        {data.headerData.map((item, i) => {
                            ...
                            if (reference.maxColumnsToShow && hideColumn && hideColumn.indexOf(i) > -1) return;

                            return (
                                <th className={i === 0 ? 'align-left' : 'align-right'} key={Utils.generateId()}>
                                    {i === 0 && !searchOpened && (
                                        <button className="custom-table-search-button" type="button" onClick={toggleSearch}>
                                            <i className="far fa-search" />
                                            {item.qFallbackTitle}
                                        </button>
                                    )}
                                    ...
                                    {i > 0 && reference.maxColumnsToShow && (
                                        <button className="table-header-btn" onClick={() => { setChoseColumnOpened(true); }} type="button">
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="custom-table-cell">
                                                {reference.columnIcon && i === 0 && row[reference.columnIcon - 1].qText !== 'ZZ_Sem Oferta' && (
            ...
            {reference.maxColumnsToShow && (
                <div className={choseColumnOpened ? 'table-chose-column active' : 'table-chose-column'}>
                    <div className="custom-table-top-change-column">
                        <button type="button" onClick={() => { setChoseColumnOpened(false); }}>
                            <i className="far fa-arrow-left" />
                        </button>
                        <span>
                            Mostrar na coluna preço
                        </span>
                    </div>
                    <div className="custom-table-column-list">
                        {data.headerData.map((item, i) => {
                            ...
                            if (reference.hideLastColumns && (data.headerData.length - reference.hideLastColumns - 1) < i) return;
                            return (
                                <div>
                                    <button
                                        className={hideColumn && hideColumn.indexOf(i) > -1 ? 'table-column-control' : 'table-column-control active'}
                                        type="button"
                                        onClick={() => { changeTableColumn(i); }}
                                    >
            ...
            && data.visualizationModel.model.layout.qHyperCube.qSize.qcy > data.bodyData.length
            && (
